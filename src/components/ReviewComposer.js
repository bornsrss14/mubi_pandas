...
export const ReviewComposer = ({ movieTmdb = 400, id_tmdb, onClose }) => {
  ...
  useEffect(() => {
    if (ratingRecord !== null) {
      setDraftReview((prev) => ({
        ...prev,
        rating: ratingRecord, // ← Actualizar cuando ratingRecord cambie
      }));
    }
  }, [ratingRecord]); // ← Se ejecuta cada vez que ratingRecord cambia
  ...
  const saveReview = async (reviewObject) => {
    try {
      ...
      setRatingRecord(response?.data?.rating); // Esto dispara el useEffect
      onClose();
    } catch (error) {
      console.log("failed trying to create this review, please, try later");
      alert(error.message || "Error al crear rating(╯°□°）╯");
    }
  };

  return (
    <div className="modal-overly" onClick={onClose}>
      <div
        className="margin-desktop modal-content"
      ...
      >
        <section className="cont-review section-persentage">
                ...
                >
                  ...
                  <div className="spoiler-toggle">
                    <input
                      ...
                      checked={draftReview.has_spoilers}
          ...
          >
            <button onClick={onClose} className="simple-button btn-cancel">
              CANCEL
            </button>
            <button
              onClick={() => saveReview(draftReview)}
